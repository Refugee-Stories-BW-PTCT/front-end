  ...
  return dispatch => {
    ...
    api()
      ...
      .then(res => {
        ...
        dispatch({
          type: GET_STORIES_DATA_SUCCESS,
          payload: res.data.filter(item => item.pending === 0)
        });
      })
      .catch(err => {
...
export function fetchStoriesPendingData() {
  return dispatch => {
    ...
    api()
      .get("/stories")
      .then(res => {
        console.log("RES PENDING STORIES", res);
        dispatch({
          type: GET_STORIES_PENDING_DATA_SUCCESS,
          payload: res.data.filter(item => item.pending === 1)
        });
      })
      .catch(err => {
        dispatch({
          type: GET_STORIES_PENDING_DATA_FAILURE,
